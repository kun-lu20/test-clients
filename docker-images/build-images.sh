...
do
  ...
  do
    ...
    then
      ...
      do
        DOCKER_ARCHITECTURE=$ARCH DOCKER_TAG="$DOCKER_TAG-kafka-$KAFKA_VERSION" MVN_ARGS="$MVN_ARGS -Dkafka.version=$KAFKA_VERSION" make build --directory=$KAFKA_MODULE
      done
    else
...
do
  ...
  then
    ...
    for ARCH in $ARCHITECTURES
    do
      DOCKER_ARCHITECTURE=$ARCH make build --directory=$HTTP_MODULE
    done
  else
